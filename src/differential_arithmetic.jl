...
Notice:
    The precidence goes: (:Wirtinger, :Casted, :Zero, :DNE, :One, :Thunk, :InplacableThunk, :Any)
...
for T in (:Casted, :Zero, :DNE, :One, :Thunk, :InplacableThunk, :Any)
...
Base.:*(a::Casted, b::Casted) = Casted(broadcasted(*, a.value, b.value))
for T in (:Zero, :DNE, :One, :Thunk, :InplacableThunk, :Any)
...
Base.:*(::Zero, ::Zero) = Zero()
for T in (:DNE, :One, :Thunk, :InplacableThunk, :Any)
...
Base.:+(::DNE, ::DNE) = DNE()
Base.:*(::DNE, ::DNE) = DNE()
for T in (:One, :Thunk, :InplacableThunk, :Any)
    @eval Base.:+(::DNE, b::$T) = b
    @eval Base.:+(a::$T, ::DNE) = a
...
Base.:*(::One, ::One) = One()
for T in (:Thunk, :InplacableThunk, :Any)
...
# InplacableThunk acts just like Thunk
Base.:+(a::InplacableThunk, b::InplacableThunk) = extern(a) + extern(b)
Base.:*(a::InplacableThunk, b::InplacableThunk) = extern(a) * extern(b)
for T in (:Any, )
    @eval Base.:+(a::InplacableThunk, b::$T) = extern(a) + b
    @eval Base.:+(a::$T, b::InplacableThunk) = a + extern(b)
